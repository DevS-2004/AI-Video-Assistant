...
def get_secret(key: str, default: str = "") -> str:
    """Works on both Streamlit Cloud (st.secrets) and local (.env via dotenv)."""
    try:
...
with st.sidebar:
    ...
    if input_mode == "🔗 YouTube URL":
        ...
        uploaded_file = st.file_uploader(
            "Upload Audio / Video",
            type=["mp4", "mp3", "wav", "webm", "m4a", "ogg"],
            help="Max 200 MB — mp4, mp3, wav, webm, m4a, ogg",
        )
...
if run_btn:
    # Validate input
    if input_mode == "🔗 YouTube URL" and not source.strip():
        st.error("Please enter a YouTube URL.")
    elif input_mode == "📁 Upload File" and uploaded_file is None:
...
if st.session_state.result:
    ...
    # ── Download buttons ─────────────────────────────────────────────────────────
    st.markdown("---")
    dl_col1, dl_col2, dl_col3, _ = st.columns([1.2, 1.2, 1.5, 3], gap="small")
    ...
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:5rem 2rem;text-align:center">
        ...
        <div style="color:var(--text-muted);font-size:0.85rem;max-width:420px;line-height:1.7">
            Paste a YouTube URL <strong>or</strong> upload an audio/video file in the sidebar,
        ...
        <div style="margin-top:2rem;display:flex;gap:1rem;flex-wrap:wrap;justify-content:center">
            ...
            <span class="badge badge-purple">Action Items</span>
            <span class="badge badge-cyan">Key Decisions</span>
            <span class="badge badge-green">Downloads</span>
        </div>
    </div>""", unsafe_allow_html=True)